...
public class SessionManager {
    ...
    public static boolean putAccountNumber(String account_number, Context context) {
        if (account_number != null) {
            CacheUtil.putPreferenceString(IConfig.SESSION_ACCOUNT_NUMBER, account_number, context);
        }
        return false;
    }

    public static String getAccountNumber(Context context) {
        return CacheUtil.getPreferenceString(IConfig.SESSION_ACCOUNT_NUMBER, context);
    }


    public static boolean putName(String name, Context context) {
        if (name != null) {
            CacheUtil.putPreferenceString(IConfig.SESSION_NAME, name, context);
        }
        return false;
    }

    public static String getName(Context context) {
        return CacheUtil.getPreferenceString(IConfig.SESSION_NAME, context);
    }

    public static boolean putUserID(String user_id, Context context) {
        if (user_id != null) {
            CacheUtil.putPreferenceInteger(IConfig.SESSION_USER_ID, Integer.parseInt(user_id), context);
        }
        return false;
    }

    public static String getUserID(Context context) {
        return String.valueOf(CacheUtil.getPreferenceInteger(IConfig.SESSION_USER_ID, context));
    }
